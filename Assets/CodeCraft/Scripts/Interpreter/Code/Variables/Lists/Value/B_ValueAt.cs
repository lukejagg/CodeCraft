...
{
    public class B_ValueAt : MonoBehaviour, ICompilerValue
    {
        ...
        {
            return new ValueAt(values[0]?.ConvertToValue(), values[1]?.ConvertToValue());
        }
        ...
        public void UpdateSize(int depth)
        {
            ...
            text.anchoredPosition = new Vector2(w1 + 171, 0);
            val2.anchoredPosition = new Vector2(w1 + 211, 0);
            block.sizeDelta = new Vector2(Width / scale, 76);

            theme.color = CodeUtil.ChangeValueColor(Colors.VariableValueColor, depth);
        }
