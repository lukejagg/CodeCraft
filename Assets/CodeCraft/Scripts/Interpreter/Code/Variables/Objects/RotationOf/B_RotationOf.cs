...
{
    public class B_RotationOf : MonoBehaviour, ICompilerValue
    {
        public CodeType CodeType => CodeType.Value;

        public string SaveId => "rot";
        public string Save()
        {
            return SaveId;
        }
        ...
        {
            return new RotationOf(values[0]?.ConvertToValue());
        }
        ...
        public void UpdateSize(int depth)
        {
            ...
            blockImage.color = CodeUtil.ChangeValueColor(Colors.RotationColor, depth);
        }
