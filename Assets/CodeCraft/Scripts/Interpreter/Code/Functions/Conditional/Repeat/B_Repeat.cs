...
{
    public class B_Repeat : MonoBehaviour, ICompilerCode
    {
        public CodeType CodeType => CodeType.Function;

        public string SaveId => "Repeat";
        public string Save()
        {
            return SaveId;
        }
        ...
        {
            return new Repeat(Values?[0]?.ConvertToValue(), EventUtil.RecursiveProgrammer(codeConnections[1]));
        }
