...
namespace Programmer
{
    public class Multiply : IValue
    {
        ...
        public Multiply(IValue value1, IValue value2)
        {
            this.value1 = value1;
            this.value2 = value2;
        }
        ...
        public object GetValue(CodeExecutor obj)
        {
			...
			else if (v1 is Vector3 vec1)
			{
				...
				if (v2 is Vector3Int vec2i)
					return Vector3.Cross(vec1, vec2i);
			...
			{
				if (v2 is Vector3 vec2)
					return Vector3.Cross(vec1i, vec2);
				if (v2 is Vector3Int vec2i)
					return Vector3.Cross(vec1i, vec2i);
